...
import {
  editCharacter,
  initEditCharacter,
} from "src/app/state/actions/edit-character.action";
import {
  deleteCharacter,
  initDeleteCharacter,
} from "src/app/state/actions/delete-character.action";
...
export class EditCharacterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.deleteCharacterSuccess$ = this.store.select(
      selectDeleteCharacterSuccess
    );

    this.store.dispatch(loadingCharacter({ id: this.id }));

    this.store.select(selectCharacter).subscribe({
      next: (character) => {
        if (character) {
